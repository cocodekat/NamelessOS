...
.PHONY: run
run: $(IMAGE_NAME).iso storage.img
	qemu-system-x86_64 \
		-M q35 \
...
.PHONY: run-video
run-video: $(IMAGE_NAME).iso storage.img
	qemu-system-x86_64 \
		-M q35 \
		-m 2G \
		-cdrom $(IMAGE_NAME).iso \
...
.PHONY: run-hdd
run-hdd: $(IMAGE_NAME).hdd storage.img
	qemu-system-x86_64 \
		-M q35 \
		-hda $(IMAGE_NAME).hdd \
		-drive id=storage,file=storage.img,format=raw,if=none \
...
.PHONY: run-hdd-uefi
run-hdd-uefi: edk2-ovmf-bins $(IMAGE_NAME).hdd storage.img
	qemu-system-x86_64 \
		-M q35 \
		-drive if=pflash,unit=0,format=raw,file=edk2-ovmf-bins/ovmf-code-x86_64.fd,readonly=on \
		-hda $(IMAGE_NAME).hdd \
...
storage.img:
	dd if=/dev/zero of=storage.img bs=1M count=8
...
distclean: clean
	...
	rm -rf limine-binary edk2-ovmf-bins storage.img